...
def MANSURA_SUBSCRIBE(username):
    ...
    user_balance = GET_USER_BALANCE(username) # GET BALANCE
    if isinstance(user_balance, float) or isinstance(user_balance, int):
        ...
        print("wasnt even able to check the price, something else broke")
...
def FILE_INSERT(uploader, uploaderId, size, post_foreign_id_source="None", 
    ...
    # BEGINNING EXOCUTION
    #TODO: CAN PROBABLY DELETE THIS WHOLE FILE SIZE CHECK THING
...
def CHECK_FILES_NOT_OVER_LIMIT(page_no, how_many):
    conn = connection.test_connection()
    print("CHECK_FILES_NOT_OVER_LIMIT", page_no, how_many)
    cursor = conn.cursor()
    cursor.execute(f"""
        SELECT * 
        FROM FILES 
